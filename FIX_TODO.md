...
- [ ] **17.** 统一 API 响应格式，删除 `ApiResponse`，统一用 `Result`
- [ ] **18.** 删除重复的 `dto.PageResult`，统一用 `common.PageResult`
- [ ] **19.** 提取 QueryController/ConversationController 指标匹配逻辑到 Service
- [ ] **20.** 前端 Dashboard 页面添加 Loading 状态
- [ ] **21.** 前端 Dashboard ECharts 实例 dispose（修复内存泄漏）
- [ ] **22.** 统一 HTTP 客户端，删除双客户端
- [ ] **23.** 修复 API 路径前缀不一致
- [ ] **24.** 导出接口添加大小限制
- [ ] **25.** SSO 回调 URL 从配置读取，token 用 fragment 传递
- [ ] **26.** Controller 直接注入 Mapper 改为通过 Service 调用（QueryController）
- [ ] **27.** 修复 fire-and-forget 异步调用（history/favorite 页面 await）
- [ ] **28.** WebSocket 断线重连
- [ ] **29.** SSE 解析支持多行 data
...
- [ ] **35.** Dashboard 清空画布添加确认弹窗
- [ ] **36.** 修复 `handleDialogClose` 破坏响应式问题
- [ ] **37.** 删除死代码（未使用的 store、重复路由）
- [ ] **38.** 响应式适配（Dashboard/Admin 页面）
- [ ] **39.** 统一 CSS 变量（消除硬编码色值）
