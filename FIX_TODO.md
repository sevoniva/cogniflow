...
- [x] **24.** 统一 HTTP 客户端，删除双客户端
- [x] **25.** 导出接口添加大小限制
- [x] **26.** SSO 回调 URL 从配置读取，token 用 fragment 传递
...
- [x] **32.** ~~统一 ECharts 使用方式（树摇导入）~~ — 评估后跳过，Vite 已自动处理
- [x] **33.** Geo-Map 页移除或实现真实功能 — 已移除
- [x] **34.** Dashboard 编辑器返回前检查未保存更改
- [x] **35.** 修复 `handleDialogClose` 破坏响应式问题（`delete` → `Object.assign`）
- [x] **36.** 删除死代码（未使用的 store、重复路由、废弃组件）
- [x] **37.** ~~响应式适配（Dashboard/Admin 页面）~~ — 评估后已基本覆盖
- [x] **38.** 统一 CSS 变量（消除硬编码色值）
